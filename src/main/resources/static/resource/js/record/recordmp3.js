	...
	var MP3Recorder = function(config) {
		...
		if (navigator.getUserMedia) {
			navigator.getUserMedia({
				...
				var context = new AudioContext(), microphone = context.createMediaStreamSource(stream), processor = context.createScriptProcessor(16384, 1, 1), // bufferSize大小，输入channel数，输出channel数
				...
				var realTimeWorker = new Worker('../../js/record/worker-realtime.js');
...
function funStartMp3() {
	recorder.start();
}

function funStopMp3(fun) {
	recorder.stop();
	recorder.getMp3Blob(function(blob) {
		...
		xhr.open('POST', getRootPath() + 'file/uploadAudio.do');
		xhr.send(fd);
	});
}